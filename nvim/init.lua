...
-- Set leader key to Space (must be set early as it affects keymaps)
vim.g.mapleader = " "
...
vim.opt.clipboard = "unnamed"           -- Yank and paste outside of vim
vim.opt.mouse = "a"                     -- Scroll using a mouse
vim.opt.number = true                   -- Show line numbers

-- Buffer behavior - allows buffers to exist in background without window
vim.opt.hidden = true

-- Enable project-level vimrc (use .exrc files for project-specific settings)
vim.opt.exrc = true
vim.opt.secure = true                   -- Disables autocmds for project-specific vimrc
...
vim.opt.colorcolumn = "90"              -- Show the line ruler
...
-- Always show the signcolumn for plugins (e.g., coc.vim, gitgutter)
...
-- Update time for better responsiveness (e.g., gitgutter updates)
...
-- Tells neovim where python executables are located
...
require("lazy").setup({
  ...
  { 
    ...
    opts = {
      keymap = {
        --   Press <c-y> to accept ([y]es) completion
        ...
        --
        -- Read `:help ins-completion` to understand why 'default' is recommended
        --
        --
  ...
  { -- File navigation with split windows
...
-- When programming, we're usually interested in working with text rather than empty space
-- To get traditional beginning of line, use ^
vim.keymap.set('n', '0', '^', { noremap = true })
vim.keymap.set('n', '^', '0', { noremap = true })

-- When typing a string, quotes auto complete. Move past the quote
-- while still in insert mode by hitting Ctrl-a
-- Example: type 'foo<c-a> -> you'll get 'foo' with cursor after the quote
